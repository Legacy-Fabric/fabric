...
public class ChildCommandElementExecutor extends CommandElement implements CommandExecutor {
		...
		if (optionalCommandMapping.isPresent()) {
			...
			try {
				...
				if (ex instanceof ArgumentParseException.WithUsage) {
					...
					throw new ArgumentParseException.WithUsage(ex, ChatMessage.createTextMessage(key + " " + ((ArgumentParseException.WithUsage) ex).getUsage()));
				}

				throw new ArgumentParseException.WithUsage(ex, ChatMessage.createTextMessage(key + " " + mapping.getCallable().getUsage(source)));
			}
		} else {
	...
	public ChatMessage getUsage(PermissibleCommandSource src) {
		...
		if (elementUsage.toString().isEmpty()) {
			return usage;
		}
