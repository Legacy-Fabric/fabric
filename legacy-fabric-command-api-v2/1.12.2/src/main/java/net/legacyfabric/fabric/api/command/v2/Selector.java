...
public enum Selector {
	...
	NEAREST_PLAYER('p') {
		@Override
		public Set<Entity> resolve(CommandSource sender) {
			return Sets.newHashSet(sender.getWorld().method_11478(sender.getPos().x, sender.getPos().y, sender.getPos().z, 50.0D, true));
		}
	},
	RANDOM_PLAYER('r') {
		@Override
		public Set<Entity> resolve(CommandSource sender) {
			return Sets.newHashSet(sender.getMinecraftServer().getPlayerManager().getPlayers().stream().findAny().orElseThrow(NullPointerException::new));
		}
	},
	EXECUTING_ENTITY('s') {
		@Override
