...
public enum Selector {
	...
	NEAREST_PLAYER('p') {
		@Override
		public Set<Entity> resolve(CommandSource sender) {
			return Sets.newHashSet(sender.getCommandSourceWorld().getNearestPlayer(sender.getCommandSourcePos().x, sender.getCommandSourcePos().y, sender.getCommandSourcePos().z, 50.0D, true));
		}
	},
	RANDOM_PLAYER('r') {
		@Override
		public Set<Entity> resolve(CommandSource sender) {
			return Sets.newHashSet(sender.getServer().getPlayerManager().getAll().stream().findAny().orElseThrow(NullPointerException::new));
		}
	},
	EXECUTING_ENTITY('s') {
		@Override
