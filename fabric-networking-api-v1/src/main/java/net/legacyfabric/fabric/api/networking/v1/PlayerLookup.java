...
public final class PlayerLookup {
	...
	public static Collection<ServerPlayerEntity> world(ServerWorld world) {
		...
		// return an immutable collection to guard against accidental removals.
		return world.getPlayers(ServerPlayerEntity.class, serverPlayerEntity -> true);
	}
	...
	public static Collection<ServerPlayerEntity> tracking(Entity entity) {
		...
		if (entity.world instanceof ServerWorld) {
			return Optional.of(entity.world)
					...
					.map(EntityTrackerAccessor::getField_31686)
					.map(c -> c.method_34005(entity.getEntityId()))
					.map(EntityTrackerEntryAccessor.class::cast)
					.map(EntityTrackerEntryAccessor::getField_31790)
