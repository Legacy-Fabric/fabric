...
public class EntityEventsTest implements ModInitializer {
	...
	public void onInitialize() {
		...
		ServerEntityWorldChangeEvents.AFTER_ENTITY_CHANGE_WORLD.register((originalEntity, newEntity, origin, destination) -> {
			LOGGER.info("%s went from dim %s to dim %s", Entities.getName(newEntity), origin.dimension.getType().getKey(), destination.dimension.getType().getKey());
		});
		ServerEntityWorldChangeEvents.AFTER_PLAYER_CHANGE_WORLD.register((player, origin, destination) -> {
			LOGGER.info("Player went from dim %s to dim %s", origin.dimension.getType().getKey(), destination.dimension.getType().getKey());
		});
