...
public class EntityEventsTest implements ModInitializer {
	...
	public void onInitialize() {
		ServerEntityCombatEvents.AFTER_KILLED_OTHER_ENTITY.register((entity, killedEntity) -> {
			LOGGER.info("{} killed {}", entity.getName(), killedEntity.getName());
		});
		ServerEntityWorldChangeEvents.AFTER_ENTITY_CHANGE_WORLD.register((originalEntity, newEntity, origin, destination) -> {
			LOGGER.info("{} went from dim {} to dim {}", newEntity.getName(), origin.dimension.getName(), destination.dimension.getName());
		});
